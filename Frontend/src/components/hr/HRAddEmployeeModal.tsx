...
const AddEmployeeModal: React.FC<AddEmployeeModalProps> = ({ onClose, onCreate, existingDepartments }) => {
  ...
  const departments = useMemo(() => (existingDepartments && existingDepartments.length > 0
    ? existingDepartments
    : ['HR','FINANCE','ORDER MANAGEMENT','COLLECTION AND DISTRIBUTION','INVENTORY']
  ), [existingDepartments]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const message = (data && data.message) ? data.message : 'Failed to create employee';
      ...
      alert(err?.message || 'Unexpected error while creating employee');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                className={`w-full px-3 py-2 border rounded-lg ${touched.employee_id && errors.employee_id ? 'border-red-500' : 'border-gray-300'}`}
            ...
            <div>
              ...
              <input
                className={`w-full px-3 py-2 border rounded-lg ${touched.nic && errors.nic ? 'border-red-500' : 'border-gray-300'}`}
                value={form.nic}
            ...
            <div>
              ...
              <input
                type="email"
                className={`w-full px-3 py-2 border rounded-lg ${touched.email && errors.email ? 'border-red-500' : 'border-gray-300'}`}
                value={form.email}
